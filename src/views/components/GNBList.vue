...
onMounted(() => {
  ...
  for (var i = 0; i < menues.value.length; i++) {
    if (menues.value[i].path != undefined) {
      //  if (menues.value[i].path == currentPath) {
      if (currentPath.includes(menues.value[i].path)) {
        current.value = i;
        break;
      }
    }
...
.menu-depth2.twoline::before,
.menu-depth3.twoline::before {
  line-height: 200%;
}
...
.menu-depth1:hover.icon::after,
.menu-depth2:hover.icon::after,
.menu-depth3:hover.icon::after {
