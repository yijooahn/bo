<template>
  <div class="modal-container">
    ...
    >
      ...
      >
        <slot name="body"></slot>
        <slot name="masking"></slot>
      </div>
      <div class="pop-footer">
        <div class="pop-footer-btns" v-if="alertModal.masking">
          <button class="btn-1" @click="clickListener('masking')">
            마스킹 해제
          </button>
          <button class="btn-2" @click="clickListener('close')">취소</button>
        </div>
        <div class="pop-footer-btns" v-else>
          <button
            v-for="btn in alertModal.buttons"
            :key="btn.index"
            :class="btn.addClass"
            @click="clickListener(btn.clickEvent)"
          >
            {{ btn.content }}
          </button>
        </div>
