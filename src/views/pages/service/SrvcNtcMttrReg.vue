...
const srvcNtcTrgt = ref("공통");
const srvcNtcTtl = ref();
const expsrMthd = ref("노출");
const uptFixYn = ref("고정아님");
const srvcNtcCn = ref();
const fileInput = ref();
...
const triggerSubmit = async () => {
  ...
  try {
    await axios.post("/servicenotice/create", formData, {
      headers: {
...
const changeDate = () => {
  if (expsrMthd.value == "P") {
    expsrBgngYmd.value = new Date().toJSON().slice(0, 10);
    expsrEndYmd.value = new Date().toJSON().slice(0, 10);
  } else {
    expsrBgngYmd.value = "";
    expsrEndYmd.value = "";
  }
