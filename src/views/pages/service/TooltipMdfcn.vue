<template>
  <div>
    ...
    <component
      ...
      @onConfirmHandler="router.push('tooltipMng')"
...
watch(tooltipTrgt, async (newVal, oldVal) => {
...
async function triggerSubmit() {
  ...
  try {
    await axios.post("/tooltip/modify", tooltipDto);
...
const inquiry = async () => {
  if (history.state.id == null) {
    ...
    tDto.maskFlag = false;
    try {
      const response = await axios.post(`/tooltip/detail`, tDto);
      const dto = response.data.tooltipDto;
      currentId.value = dto.tooltipId;
      tooltipTrgt.value = dto.tooltipTrgt == "TCHR" ? "교사용" : "학생용";
      tooltipTtl.value = dto.tooltipTtl;
      pblcnUseYn.value = dto.pblcnUseYn == "Y" ? "기간설정" : "기한없음";
      pblcnBgngYmd.value = dto.pblcnBgngYmd;
      pblcnEndYmd.value = dto.pblcnEndYmd;
      tooltipCn.value = dto.tooltipCn;
      await getTextbookCodeList(subjectList, selectedSubjectNm);
      await getUiCodeList("/tooltip", tooltipTrgt, uiList, selectedUiNm);
      selectedSubjectNm.value = dto.comCdNm;
      selectedUiNm.value = dto.uiTtl;
    } catch (error) {
      if (error.response.status != null) {
        router.push("/tooltipMng");
      }
