<template>
    <div class="wrap" ref="wrap">
		<p class="category">서비스 관리 > 이용 상세정보</p>
		<p class="title">이용가이드 상세정보</p><br>
		...
		<table>
			...
			<tr>
				...
				<td v-if="userGdTrgt === 'TCHR'"><span>교사용</span></td>
				<td v-if="userGdTrgt === 'STDNT'"><span>학생용</span></td>
			</tr>
			<tr>
				<th class="align-left"><span>제목</span></th>
				<td><span>{{ userGdTtl }}</span></td>
			</tr>
			<tr>
				<th class="align-left"><span>게재 기간</span></th>
				<td v-if="pblcnUseYn === 'Y'"><span>{{ pblcnBgngYmd }} ~ {{ pblcnEndYmd }}</span></td>
				<td v-if="pblcnUseYn === 'N'"><span>기한없음</span></td>
			</tr>
			<tr>
				<th class="align-left"><span>현황</span></th>
				<td v-if="userGdStts === '현재버전'"><span class="text-danger">{{ userGdStts }}</span></td>
				<td v-if="userGdStts === '종료'"><span>{{ userGdStts }}</span></td>
			</tr>
			<tr>
				<th class="align-left"><span>파일첨부</span></th>
				<td v-if="atchFile != null"><img src="/images/icon06.svg">
					<span class="underline" style="vertical-align: super;">{{atchFile}}</span>
				</td>
				<td v-if="atchFile ===null"><span>첨부된 파일이 없습니다</span></td>
			</tr>
			<tr>
				<th><span>내용</span></th>
				<td><span>{{ userGdCn }}</span></td>
			</tr>
		</table><br>
		
		<div class="tit"></div><label class="label-tit">등록/수정 정보</label>
		<table>
			<colgroup>
				<col width="10%">
				<col width="40%">
			...
			<tr>
				<th><span>등록자</span></th>
				<td><span>{{rgtr}}</span></td>
				<th><span>등록일</span></th>
				<td><span>{{regDt}}</span></td>
			</tr>
			<tr>
				<th><span>수정자</span></th>
				<td><span>{{chgr}}</span></td>
				<th><span>수정일</span></th>
				<td><span>{{chgDt}}</span></td>
			</tr>
		</table><br>
		<div class="align-center">
			<button class="btn-1" @click="moveToModification()">수정</button>
...
const userGdId = history.state.userGdId;
...
const inq = async () => {
	await axios.get("/userguide/detail/"+userGdId)
    	.then((response) => {
			...
			rgtr.value = response.data.userguideDto.rgtr
			chgr.value = response.data.userguideDto.chgr
			regDt.value = response.data.userguideDto.regDt
			chgDt.value = response.data.userguideDto.chgDt
