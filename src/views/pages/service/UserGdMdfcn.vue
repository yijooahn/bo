<template>
    <div class="wrap" ref="wrap">
		<p class="category">서비스 관리 > 이용 수정</p>
		<p class="title">이용가이드 수정</p><br>
		...
		<table>
			...
			<tr>
				...
				<td><input type="text" v-model="userGdTtl" style="display:inline-block;width:85.5%;" placeholder="제목을 입력하세요"></td>
			</tr>
			<tr>
				<th class="align-left"><span>게재 기간</span><span class="text-danger"> *</span></th>
				<td><select v-model="pblcnUseYn">
			...
			<tr>
				...
				<td>
					<div v-if="atchFile != null" style="display:inline-flex; vertical-align: middle;">
						<img src="/images/icon06.svg"><span>{{ atchFile }}</span><img src="/images/icon07red.svg" class="clickable" @click="rmvFile()">&emsp;
					</div>
					<div v-if="atchFile === null" class="dp-if"><span>첨부된 파일이 없습니다</span></div>&emsp;
			...
			<tr>
				...
				<td><textarea rows="5" v-model="userGdCn">{{ userGdCn }}</textarea></td>
			</tr>
		</table><br>
		
		<div class="align-center">
			<button class="btn-1" @click="handleClick()">수정</button>
...
let userGdId = history.state.userGdId;
...
const chgrId = ref()
const inq = () => {
	axios.get("/userguide/detail/"+userGdId)
    	.then((response) => {
			userGdTrgt.value = response.data.userguideDto.userGdTrgt
			userGdTtl.value = response.data.userguideDto.userGdTtl
			userGdUseYn.value = response.data.userguideDto.userGdUseYn
			userGdStts.value = response.data.userguideDto.userGdStts
			pblcnUseYn.value = response.data.userguideDto.pblcnUseYn
			pblcnBgngYmd.value = response.data.userguideDto.pblcnBgngYmd
			pblcnEndYmd.value = response.data.userguideDto.pblcnEndYmd
			userGdCn.value = response.data.userguideDto.userGdCn
			if(response.data.atchFileDtoList.length > 0){
				atchFile.value = response.data.atchFileDtoList[0].atchFileNm
				atchFileTmp = response.data.atchFileDtoList[0].atchFileNm
...
const rmvFile = () => {
	axios.get("/atchFile/removeFile/"+atchFileId.value
		).then(() => {
			atchFile.value = null
			atchFileId.value = null
		})
} 
...
const handleClick = async () => {
	...
	|| userGdTrgt.value === '' || userGdTtl.value === '' || userGdCn.value === ''){
		confirmPopupContent.value = ["입력되지 않은 항목이 있습니다.","확인 후 다시 시도해 주십시오."]
		const ok = await confirmPopup.value.show();
	} else {
		yesNoPopupContent.value = ["수정 하시겠습니까?"]
...
const modify = async() => {
	...
	if(atchFile.value === null || atchFile.value != atchFileTmp) {
		if(atchFileId.value != '' && atchFileId.value != null && atchFileId.value != undefined) {
			await axios.get("/atchFile/removeFile/"+atchFileId.value)
		}
	}
	if(pblcnUseYn.value == 'N') {
		pblcnBgngYmd.value = null
		pblcnEndYmd.value = null
	}
	axios.post("/userguide/modify",formData, {headers: {'Content-Type': 'multipart/form-data'},
									params:{'userGdId':userGdId,
											'userGdTrgt':userGdTrgt.value,
											'userGdTtl':userGdTtl.value,
											'userGdUseYn':userGdUseYn.value,
